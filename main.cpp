    ...
    if (res.is_ok()) {
        for (auto &row : res) {
            ...
            for (uint8_t byte : password_hash) {
                std::cerr << std::hex                    // переключаем на hex формат
                          << std::setw(2)                // минимальная ширина 2 символа
                          << std::setfill('0')           // заполнять нулями
                          << static_cast<int>(byte);     // приводим к int, чтобы вывести как число
            }
